...
Try {
	...
	[string]$appVendor = 'Ricoh'
	[string]$appName = 'DocuWare'
	[string]$appVersion = '7.6.0.4264'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '2022.10.12'
	...
	$CloseApps = "DocuWare.Desktop=DocuWare"

	##* Application install parameters.
	# 64-bit application
	# Application install Path.
	$64bitAppInstallPath = "$PSScriptRoot\Files\DTA75Cloud Silent Install.ps1"

	##* Application uninstall parameters.
	# 64-bit application
	# Application uninstall path.
	$64bitAppUninstallPath = "$PSScriptRoot\Files\DTA75Cloud Silent Uninstall.ps1"
	...
	##* Application unpdate parameters.
	# 64-bit application
	# Application uninstall path.
	$64bitAppUpdatePath = "$PSScriptRoot\Files\DTA75Cloud Silent Update.ps1"
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		# Check if previous EXE versions of application are installed.  
		If (Test-Path -Path "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\DocuWare") {
			# Uninstall previous EXE versions of application(s).      
			Try {
				Execute-Process -Path "$PSHOME\powershell.exe" -Parameters "-file `"$64bitAppUninstallPath`"" -Wait
			}
			# If uninstall failed, log results. Exit script.
		...
		Try {
			...
			Execute-Process -Path "$PSHOME\powershell.exe" -Parameters "-file `"$64bitAppInstallPath`"" -Wait
			Execute-Process -Path "$PSHOME\powershell.exe" -Parameters "-file `"$64bitAppUpdatePath`"" -Wait
